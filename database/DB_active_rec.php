...
abstract class CI_DB_active_record extends CI_DB_driver {
	...
	// Active Record Caching privateiables
	 ...
	 * @return	object
	 */
	private function _max_min_avg_sum($select = '', $alias = '', $type = 'MAX')
	{
	 ...
	 * @return	string
	 */
	private function _create_alias_from_table($item)
	{
	 ...
	 * @return	object
	 */
	private function _where_in($key = NULL, $values = NULL, $not = FALSE, $type = 'AND ')
	{
	...
	public function delete($table = '', $where = '', $limit = NULL, $reset_data = TRUE)
	{
		...
		if (count($this->ar_where) == 0 AND count($this->ar_wherein) == 0 AND count($this->ar_like) == 0)
		{
			if ($this->db_debug)
			{
	...
	private function _object_to_array($object)
	{
		...
		foreach (get_object_vars($object) as $key => $val)
		{
			...
			if ( ! is_object($val) AND ! is_array($val) AND $key != '_parent_name' AND $key != '_ci_scaffolding' AND
				$key != '_ci_scaff_table')
			{
				$array[$key] = $val;
			}
	...
	protected abstract function _from_tables($table);
