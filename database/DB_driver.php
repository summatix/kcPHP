...
abstract class CI_DB_driver {
	...
	{
		return preg_match('/^\s*"?(SET|INSERT|UPDATE|DELETE|REPLACE|CREATE|DROP|TRUNCATE|LOAD DATA|COPY|ALTER|GRANT|REVOKE|LOCK|UNLOCK)\s+/i', $sql) == TRUE;
	}
	...
	public function list_fields($table = '')
	{
		...
		if (($sql = $this->_list_columns($this->_protect_identifiers($table, TRUE, NULL, FALSE))) === FALSE)
		{
			if ($this->db_debug)
			{
				return $this->display_error('db_unsupported_function');
	...
	{
		$str = trim($str);
		return preg_match('/(\s|<|>|!|=|is null|is not null)/i', $str) == TRUE;
	}
	...
	private function _cache_init()
	{
		...
		{
			$file = BASEPATH.'database/DB_cache.php';
			if ( ! file_exists($file))
			{
	...
	protected abstract function db_connect();
	protected abstract function db_pconnect();
	protected abstract function db_select();
	protected abstract function _execute($sql);
	protected abstract function _list_tables($prefix_limit = FALSE);
	protected abstract function _list_columns($table = '');
	protected abstract function _field_data($table);
	protected abstract function _error_message();
	protected abstract function _error_number();
	protected abstract function _escape_identifiers($item);
	...
	protected abstract function _db_set_charset($charset, $collation);
}
